...
function renderProducts() {
    ...
    sortedProducts.forEach(product => {
        ...
        row.innerHTML = `
            <td>${product.name}</td>
            <td>${formatNumber(product.price)} ریال</td>
            <td>
...
function renderInventory() {
    ...
    const inventory = sortedProducts.map(product => {
        ...
        const totalPrice = finalStock * (product.price || 0);
    ...
    inventory.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td style="font-weight: 600;">${formatNumber(item.totalPrice)} ریال</td>
        `;
        tbody.appendChild(row);
    });
    
    // Calculate total assets
    const totalAssets = inventory.reduce((sum, item) => sum + item.totalPrice, 0);
    ...
    if (inventory.length > 0) {
        ...
        summaryRow.innerHTML = `
            <td>مجموع دارایی‌ها</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td>-</td>
            <td style="color: #007AFF; font-size: 1.1em;">${formatNumber(totalAssets)} ریال</td>
        `;
        ...
        if (e.target.hasAttribute('data-action') && e.target.getAttribute('data-action') === 'save') {
            ...
            if (productModal.dataset.action === 'add') {
                ...
                if (product) {
                    ...
                    const productPriceInput = document.getElementById('product-price');
                    product.price = parseFloat(productPriceInput?.value) || 0;
...
function initializeApp() {
    ...
    if (appState.products.length === 0) {
        appState.products = [
            { id: generateId(), name: 'محصول نمونه 1', price: 50000 },
            { id: generateId(), name: 'محصول نمونه 2', price: 75000 },
            { id: generateId(), name: 'محصول نمونه 3', price: 100000 }
        ];
        saveData();
        renderProducts();
        refreshProductSelects();
